...
\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth]{figures/lil_qualitative_example.pdf}
\caption{Qualitative LIL example. \textbf{(A)} Raw prompt with implicit boundaries.
\textbf{(B)} LIL wire format with explicit role markers and macro compaction.
\textbf{(C)} Tokenization stability: LIL isolates extension from core instruction,
preserving token stability (green) while baseline exhibits churn (red).}
